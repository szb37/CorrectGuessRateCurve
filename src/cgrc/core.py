...
class Controllers():
    ...
    def get_cgrc_comparison_table_v2(trial_name, analysis_name, cgrc_data_dir, cgrc_stats_dir, trial_scales=None,):
        ...
        for scale, cgr in product(cgrc_data.scale.unique().tolist(), cgrc_data.cgr.unique().tolist()):
            ...
            tmp_cgrc_data = cgrc_data.loc[(
                cgrc_data.scale == scale) & (cgrc_data.cgr == cgr)]
            tmp_cgrc_stats = cgrc_stats.loc[(
                cgrc_stats.scale == scale) & (cgrc_stats.cgr == cgr)]
    ...
    def get_cgrc_datapoint_KDE(df_filtered, sample_sizes, cgr, kdes):
        ''' Generate CGRC datapoints from KDEs
            Args:
                ...
                kdes (dict): dictionary of KDE objects
