...
class Drawer():
    ...
    def draw_vsCGR(ax1, ax2, title, cgrc_model_comps, cgr, add_legend=True):
        ...
        if constants.estimator == 'mean':
            ...
            sns.lineplot(ax=ax2, x='cgr', y='est',
                         estimator=np.median, color=color2, data=data)
        else:
            assert False
        ...
        ax1.yaxis.set_major_locator(
            ticker.LinearLocator(9))  # originally was 9
        ...
        # Vertical line, original CGR
        ax1.axvline(x=cgr, color='green', ls='--')
        ax1.axvline(x=0.5, color='black', ls='--')  # Vertical line, True CGR
        # Horizontal line, significance threshold
        ax1.axhline(y=0.05, color='red', ls='--')
